...
LR=1e-5
RUN_NAME=rl_hammer_target_gemini_2_5_flash_lora
...
accelerate launch \
    ...
    --target_model_name_or_path "gemini-2.5-flash-preview-05-20;meta-llama/Llama-3.1-8B-Instruct" \
...
for dir in checkpoints/${RUN_NAME}/*; do
    if [ -d "$dir" ]; then
        python injecagent_eval.py \
            ...
            --target_model_name_or_path gemini-2.5-flash-preview-05-20 \
            --validation_data_path data/InjecAgent/dataset/eval.json \
            --enable_wandb True \
            --run_name eval_${RUN_NAME}_attack_gemini_2_5_flash
    fi
done
